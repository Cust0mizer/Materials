...
В C# используется именно `callvirt` для генерации исключений `NullReferenceException` и даже для методов, в которых вообще не нужен экземпляр класса, например в методе:
```csharp
	public void GetFive()
	{
		return 5;
...
Где-бы не находился этот метод, он просто возвращает 5, и никак не обращается к экземпляру класса, не использует его поля, но все равно при попытке обратиться к классу, мы получим ошибку NullRef
